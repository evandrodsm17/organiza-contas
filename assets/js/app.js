...
function loginModal() { return `<div class="modal-backdrop auth-backdrop"><form class="modal login-modal" id="loginForm"><button class="modal-close" type="button" id="closeModal" aria-label="Fechar">×</button><div class="auth-brand"><span class="brand-mark">O</span><span>OrganizaContas</span></div><span class="eyebrow">ÁREA SEGURA</span><h2>Bem-vindo de volta</h2><p>Acesse seu espaço financeiro compartilhado.</p><label>E-mail<input name="email" type="email" autocomplete="username" placeholder="voce@exemplo.com" required></label><label>Senha<input name="password" type="password" autocomplete="current-password" placeholder="Digite sua senha" required></label><button class="btn btn-primary wide" type="submit">Entrar na minha conta</button><small class="auth-note">Acesso exclusivo para usuários autorizados.</small><small id="configHint"></small></form></div>`; }
function bindLogin() {
...
function calendarStatus(item) { if (item.type === "income") return "income"; return item.status === "paid" ? "expense-paid" : "expense-pending"; }
