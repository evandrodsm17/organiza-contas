...
function observeManagements() {
  stopManagements = FirebaseService.observeManagements(state.user.uid, (items) => {
    ...
    state.selected = items.find((item) => item.id === selectedId) || items[0] || null;
...
function renderApp() {
  ...
  ${state.view === "users" ? renderUsers() : state.view === "settings" ? renderSettings(monthName) : state.view === "calendar" ? renderCalendar(monthName) : state.view === "records" ? renderRecords(monthName) : renderDashboard(monthName)}</main></div><div id="modalRoot"></div>`;
  bindShell();
}

function nav(id, iconName, label) { const active = state.view === id || (id === "settings" && state.view === "users"); return `<button class="nav-item ${active ? "active" : ""}" data-view="${id}">${icon(iconName)}<small>${label}</small></button>`; }
function viewTitle() { return ({ dashboard: "Visão geral", calendar: "Calendário financeiro", records: "Lançamentos", settings: "Configurações", users: "Gestão de usuários" })[state.view]; }
...
function renderBudgetScore(expense) {
  ...
  if (!limit) return `<article class="panel budget-score-panel budget-score-empty"><div class="panel-head"><div><h2>Score financeiro</h2><p>Uso do limite de despesas</p></div></div><div class="budget-empty-indicator"><span>—</span><small>Limite não definido</small></div><p>Defina quanto pretende gastar neste mês para acompanhar seu planejamento.</p><button class="text-btn" type="button" data-go="settings">Definir limite mensal</button></article>`;
  const expenseCents = Math.round(expense * 100); const limitCents = Math.round(limit * 100);
  const usage = Math.max(0, (expenseCents / limitCents) * 100); const percent = Math.round(usage); const progress = Math.min(usage, 100).toFixed(2); const remaining = (limitCents - expenseCents) / 100;
  let tone = "safe"; let label = "Dentro do planejado";
  if (expenseCents > limitCents) { tone = "over"; label = "Limite excedido"; }
  else if (expenseCents === limitCents) { tone = "limit"; label = "No limite definido"; }
  else if (usage >= 80) { tone = "warning"; label = "Próximo do limite"; }
...
function renderSettings(monthName) {
  const limit = expenseLimitForMonth(); const currentExpense = state.selected ? totals().expense : 0;
  const canCreate = state.profile.canCreateManagement || state.profile.role === "master"; const preference = themePreference();
...
function bindShell() {
  ...
  document.querySelectorAll("[data-go]").forEach((button) => button.onclick = () => { state.view = button.dataset.go; if (state.view === "users") observeUsers(); renderApp(); });
  ...
  document.querySelectorAll("[data-theme-choice]").forEach((button) => button.onclick = () => setThemePreference(button.dataset.themeChoice));
  const expenseLimitForm = document.querySelector("#expenseLimitForm");
  if (expenseLimitForm) expenseLimitForm.onsubmit = async (event) => {
    event.preventDefault(); const button = expenseLimitForm.querySelector('button[type="submit"]'); const limit = Number(expenseLimitForm.limit.value);
    if (!limit || limit <= 0) { toast("Informe um limite mensal maior que zero.", "danger"); return; }
    busy(button, true);
    try { await FirebaseService.setMonthlyExpenseLimit(state.selected.id, state.month, limit); toast("Limite mensal atualizado."); }
    catch (error) { firebaseError(error); busy(button, false); }
  };
  document.querySelector("#clearExpenseLimit")?.addEventListener("click", async (event) => {
    const button = event.currentTarget; busy(button, true);
    try { await FirebaseService.setMonthlyExpenseLimit(state.selected.id, state.month, null); toast("Limite mensal removido."); }
    catch (error) { firebaseError(error); busy(button, false); }
  });
...
function themePreference() { return localStorage.getItem("organiza-theme") || "system"; }
function setThemePreference(preference) {
  if (preference === "system") { localStorage.removeItem("organiza-theme"); document.documentElement.dataset.theme = matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light"; }
  else { localStorage.setItem("organiza-theme", preference); document.documentElement.dataset.theme = preference; }
