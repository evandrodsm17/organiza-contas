...
function openManagementModal() { showModal(`<form class="modal" id="managementForm"><button class="modal-close" type="button">×</button><span class="eyebrow">NOVO ESPAÇO</span><h2>Criar gerenciamento</h2><label>Nome<input name="name" placeholder="Ex.: Contas de casa" required maxlength="80"></label><label>Descrição<textarea name="description" placeholder="Opcional" maxlength="240"></textarea></label><button class="btn btn-primary wide" type="submit">Criar gerenciamento</button></form>`); const form = document.querySelector("#managementForm"); form.onsubmit = async (e) => { e.preventDefault(); const button = form.querySelector("button[type=submit]"); busy(button, true); try { await FirebaseService.createManagement(Object.fromEntries(new FormData(form)), state.user); closeModal(); toast("Gerenciamento criado."); } catch (err) { firebaseError(err); busy(button, false); } }; }
